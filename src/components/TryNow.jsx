...
export function TryNow() {
  ...
  const handleCreateShort = async () => {
    ...
    return;
    if (videoUrl) {
      const { data } = await axios.post("api/generateShorts", {
        url: videoUrl,
      });
      const response = {
  ...
  return (
    <div className="mt-8">
      <div className="title-input grid">
        ...
        <div className="input-repo mx-auto flex relative">
          ...
          <button
            ...
            className="w-16 h-12 bg-green-600 absolute right-3 top-0 bottom-0 my-auto rounded-lg text-white flex justify-center items-center hover:bg-green-500 cursor-pointer shadow-md"
          >
            Go
          </button>
        </div>
      ...
      <ShowShortVideo
        video_url="https://hacks-dub-ai.s3.eu-north-1.amazonaws.com/4c6f6502-0239-4863-b079-464a1baa2ecb.mp4"
        autoPlay={false}
        isShort={false}
      />
...
function ShowShortVideo({ video_url, autoPlay, isShort = true }) {
      ...
      <div onClick={handleTogglePlay}>
        {isPlaying ? (
          <svg
            ...
            stroke-width="0"
            viewBox="0 0 512 512"
            className="absolute z-[4] fill-orange-500 flex items-center justify-center hover:fill-orange-300 duration-150 hover:scale-110 ease-in-out m-auto top-0 left-0 right-0 bottom-0"
            height="50"
            width="50"
